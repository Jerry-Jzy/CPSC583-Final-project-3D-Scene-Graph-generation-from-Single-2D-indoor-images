...
from osdsynth.utils.logger import SkipImageException, save_detection_list_and_QAs_to_json, setup_logger
...
def annotate(cfg, global_data, logger, device):
    ...
    for i, filepath in tqdm(enumerate(global_data), ncols=25):
        ...
        try:
            ...
            save_detection_list_and_QAs_to_json(detection_list, vqa_results, detection_list_path)

            print('detection size: {}'.format(len(detection_list)))
            print('QA size: {}'.format(len(vqa_results)))
