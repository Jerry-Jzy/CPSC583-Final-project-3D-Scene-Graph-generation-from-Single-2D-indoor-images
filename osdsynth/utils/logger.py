...
def save_detection_list_and_QAs_to_json(detection_list, QAs, output_file):
    ...
    def serialize_QA(QA):
        serialized = {}
        serialized['object_pairs'] = [QA[1]["caption"].lower(),QA[2]["caption"].lower()]
        serialized['spatial_relationship_type'] = QA[3]
        serialized['spatial_relationship'] = {
            'question': QA[0][0],
            'answer': QA[0][1],
        }
        return serialized
    ...
    serialized_QAs = [serialize_QA(QA) for QA in QAs]
    serialized_list_and_QAs = {"nodes": serialized_list, "edges": serialized_QAs}

    # with open(output_file, "w") as f:
    #     json.dump(serialized_list, f, indent=2)
    with open(output_file, "w") as f:
        json.dump(serialized_list_and_QAs, f, indent=2)

    print(f"Detection list and QAs saved to {output_file}")
